<!-- 
        C = Create = Insertar = Insert Into
        R = Read = Consultar = Select * From
        U = Update = Actualizar = Update SET
        D = Delete = Eliminar = Delete From
-->
<html>
    <head>
        <title>Editar Consultorio</title>
        <?php include_once $GLOBALS["UrlWeb"] . "Views/comun/header.php" ?>
    </head>    
    <body>
        <?php include_once $GLOBALS["UrlWeb"] . "Views/comun/nav-enfermero.php" ?>
        <div class="row mt-4">
            <div class="col-lg-4"></div>
            <div class="col-lg-4">
                <div class="card">


                    <form action="<?php echo $GLOBALS["UrlWebHttp"] ?>Controllers/Proxy.php?c=Consultorios&a=actualizar" method="POST">
                        <div class="card-header bg-success text-white text-center">Editar Consultorio</div>
                        <div class="card-body">
                            <label>Número del Consultorio</label><br/>
                            <input name="conNumero" type="hidden" value="<?php echo $entity->ConNumero ?>" >
                            <input name="" type="text" value="<?php echo $entity->ConNumero ?>" disabled="disabled"><br/>
                            <label>Nombre del Consultorio</label><br/>
                            <input name="conNombre" type="text" value="<?php echo $entity->ConNombre ?>"><br/><br/>
                        </div>
                        <div class="card-footer text-center"> <input type="submit" value="Editar Consultorio" class="btn btn-primary"> </div>
                    </form>   
                </div> 
            </div>
            <div class="col-lg-4"></div>
        </div>
    </body>
</html>

